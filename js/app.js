/* Hamza Food — App v6 - Enhanced Professional Edition */
...
function readSavedLanguage() {
    try {
        return localStorage.getItem(STORAGE_KEY) === 'fr' ? 'fr' : 'ar';
    } catch (error) {
        return 'ar';
    }
}
...
function translateElement(el, lang) {
    ...
    if (textNodes.length && el.children.length) {
...
function applyTranslations() {
    ...
    document.querySelectorAll('[data-aria-ar]').forEach(el => {
        const ariaLabel = el.getAttribute(`data-aria-${currentLang}`);
...
function getCategoryItems(key, cat, ignoreActiveTab = false) {
    ...
    return cat.items.filter(item => {
        return matchesMenuFilters(item, cat);
    });
}
...
function updateMenuSummary(totalVisible) {
    ...
    if (contextEl) {
        if (menuQuery) {
            contextEl.textContent = currentLang === 'ar'
                ? `نتائج: ${menuQuery}`
                : `Resultats : ${menuQuery}`;
        } else if (showFeaturedOnly) {
            contextEl.textContent = currentLang === 'ar' ? 'الأطباق المميزة فقط' : 'Plats signatures';
        } else if (activeTab === 'all') {
            contextEl.textContent = currentLang === 'ar' ? 'كل الأصناف' : 'Toutes les categories';
        } else if (MENU[activeTab]) {
            contextEl.textContent = MENU[activeTab][currentLang];
        }
    }

    if (featuredToggle) {
        featuredToggle.classList.toggle('is-active', showFeaturedOnly);
        featuredToggle.setAttribute('aria-pressed', String(showFeaturedOnly));
    }

    if (resetButton) {
        resetButton.classList.toggle('is-hidden', !(menuQuery || showFeaturedOnly || activeTab !== 'all'));
    }
}
...
function renderMenu() {
    ...
    Object.entries(MENU).forEach(([key, cat]) => {
        ...
        visibleItems.forEach((item, index) => {
            ...
            html += '<div class="relative h-40 overflow-hidden rounded-t-lg bg-navy-800"><img src="' + catImage + '" alt="' + cat[currentLang] + '" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy" onerror="this.src=\'https://images.unsplash.com/photo-1495521821757-a1efb6729352?w=400&h=300&fit=crop\'"></div>';
    ...
    if (!totalVisible) {
        html = '<div class="menu-empty"><strong>' +
            (currentLang === 'ar' ? 'ما لقيناش هاد الطبق' : 'Aucun plat trouve') +
            '</strong><p>' +
            (currentLang === 'ar' ? 'جرّب اسم آخر أو مسح البحث باش تشوف المنيو كاملة.' : 'Essayez un autre mot-cle ou effacez la recherche pour voir tout le menu.') +
            '</p></div>';
    }
...
function updateStatus() {
    ...
    if (footerBadge && footerLabel) {
        ...
        footerLabel.textContent = currentLang === 'ar'
            ? (open ? 'مفتوح الآن' : 'مغلق الآن')
            : (open ? 'Ouvert maintenant' : 'Ferme pour le moment');
    }
...
function openLightbox(src, alt) {
    ...
    overlay.innerHTML = '' +
        ...
        '<button type="button" class="lightbox-close" aria-label="' + (currentLang === 'ar' ? 'اغلاق الصورة' : 'Fermer l image') + '"><i class="fas fa-times"></i></button>' +
        '<img class="lightbox-image" src="' + src + '" alt="' + (alt || 'Hamza Food') + '">' +
        '<figcaption class="lightbox-caption">' + (alt || 'Hamza Food') + '</figcaption>' +
        '</figure>';
